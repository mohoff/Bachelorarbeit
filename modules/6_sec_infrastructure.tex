...
\label{secInfrastructure}
  ...
  \label{publicCloud}
    ...
    \subsection{Beispiel: Microsoft Azure}
      ...
      % "vielversprchend" anstatt "beliebten"? Oder ist das zu umgangssprachlich?
  ...
  \label{privateCloud}
    ...
    % Evtl. Bullets draus machen. 1. keine Trennung nach Kunden 2. Verfügbarkeit weiterhin wichtig 3. Vertraulichkeit und Integrität weniger 4. Vertraute Anwendungen/Umgebung 5. Keine böswilligen Absichten anderer Instanzen.
    ...
    % TODO: Quelle dazu (war das in einem Docker-doc?)

    % Trend zu hybrid Cloud erwähnen, und Sicherheitsfolgen. Aber Begriff noch sehr schwammig... fehlt es an konkreten teschnischen Lösungen, oder? Sagt ja eigentlich nur aus, dass public und private Cloud nahtlos ineinander integriert werden... was das aus anwendungssicher auch immer zu bedeuten hat.
    ...
    \subsection{Beispiel: OpenStack}
      ...
      Durch die Vielseitigkeit und Größe von \emph{OpenStack}, wird der Technologie eine hohe Komplexität und eine lange Einarbeitungszeit nachgesagt. \emph{SUSE} und andere Linux-Anbieter profitieren von dieser Eigenschaft, indem sie z.B. mit \emph{SUSE OpenStack Cloud} eigene \emph{OpenStack}-Lösungen anbieten, die Kunden eine einfach realisierbare Private Cloud ermöglicht
      \cite{https://www.suse.com/promo/cloud/}
      \cite{http://www.golem.de/news/openstack-viele-brauchen-es-keiner-versteht-es-wir-erklaeren-es-1503-112814-2.html}
      \cite{http://www.golem.de/news/openstack-viele-brauchen-es-keiner-versteht-es-wir-erklaeren-es-1503-112814-4.html}.
      Das Startup \emph{Platform 9} verspricht \emph{OpenStack}-as-a-Service, ein Angebot, das die Komplexität von \emph{OpenStack} für Kunden verbergen soll \cite{http://platform9.com/why-platform9/}.

      Trotz der Komplexität integrieren viele Anbieter auf dem Cloud-Markt vermehrt \emph{OpenStack} in ihr Portfolio. Unterstützer von \emph{OpenStack} sind \emph{SUSE}, \emph{Red Hat}, \emph{IBM}, \emph{Oracle}, \emph{Intel}
      \cite{http://www.heise.de/developer/meldung/OpenStack-Liberty-Cloud-Framework-macht-das-Dutzend-voll-2849083.html}
      \cite{https://opensource.com/resources/what-is-openstack}.
      % Wird als Zukunft von Cloud-Computing gehandelt.

      Auch die Entwicklung von \emph{OpenStack} ist sehr aktiv. Anfang 2015 wurde mit \emph{Projekt Magnum} ein weiteres Modul angekündigt, das Docker und \emph{Kubernetes} in \emph{OpenStack} verfügbar macht \cite{https://wiki.openstack.org/wiki/Magnum}. Davor war ein Betrieb von Containern nur über den Treiber \emph{Nova-Docker} für das Modul Nova möglich, dessen Funktionsumfang sehr begrenzt ist
      \cite{http://www.heise.de/newsticker/meldung/OpenStack-goes-Container-2660052.html}
      \cite{https://wiki.openstack.org/wiki/Docker}.
      Magnum nutzt die bestehenden Funktionen von Nova und Heat, die virtuelle Instanzen verwalten und fügt Schnittstellen hinzu, um mit Docker- und Kubernetes-Installationen innerhalb der virtuellen Instanzen zu kommunizieren \cite{https://wiki.openstack.org/wiki/Magnum}.

      Neben der neuen Integration von Magnum, unterstützt \emph{OpenStack} auch viele Hypervisor-Technologien wie KVM, XEN, Hyper-V von \emph{Microsoft} und vSphere von \emph{VMware}.
