...
\label{secLinux}
	Dieses Kapitel stellt die von dem Betriebssystem Linux ermöglichten Sicherheitsmodelle und -mechanismen vor, die Docker unterstützt. Dazu zählen zunächst die Isolierung kritischer Bereiche, die meist nach dem chroot-Prinzip in Form von sogenannten Namespaces, erfolgt. Außerdem werden Control Groups, kurz Cgroups, vorgestellt, die eine Ressourcenverwaltung implementieren. Der Linux-Kernel bietet weitere Sicherheitsfunktionen an, die beispielsweise durch die Implementierung einer Mandatory Access Control (MAC) von Docker genutzt werden. Auch sind Capability-basierte Sicherheitsfunktionen, wie sie von Linux ermöglicht und in einer Reihe an Werkzeugen Anwendung finden, Gegenstand dieses Kapitels.

	Diese Vielzahl an Mechanismen werden von Docker unterstützt, um nach den Prinzipien \emph{Defense In Depth} und \emph{Principle Of Least Privilege} einen möglichst sicheren Betrieb von Containern zu ermöglichen.

	\begin{itemize}
			\item \emph{Defense In Depth}: Es werden möglichst viele Sicherheitsmechanismen aktiviert, um die Gesamtsicherheit zu erhöhen. Der Funktionsumfang der einzelnen Mechanismen kann sich - wie auch in der vorliegenden Sicherheitsarchitektur der Fall ist - überschneiden.
	...
	Zudem bieten viele Mechanismen an, die Sicherheitskonfiguration, z.B. in der Form von Profilen, dynamisch anzupassen, sodass eine individuelle Begrenzung oder Ausdehnung von Rechten pro Containerapplikation möglich ist.

	Wie In Kapitel \ref{introVirtContainer} geschildert, wird Docker zur Zeit nur für Linux angeboten. Aus diesem Grund ist eine Untersuchung von Sicherheitsfeatures anderer Betriebssysteme in diesem Kapitel irrelevant.
  ...
  \label{secResLimit}
		...
		Es ist zu erwarten, dass die fehlenden Funktionen von Cgroups sowie die ausstehende Unterstützung der \emph{Unified Hierarchy}, im Rahmen der Integration des Cgroup-Namespace zukünftig stattfindet. Eine weitere einheitliche Verwaltung von Ressourcen auf der Basis von Cgroups ist vorgesehen \cite[S.16+19]{dockerSec2}.
