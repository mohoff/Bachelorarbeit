...
\label{secLinux}
  ...
  \label{secIsolierung}
    ...
    \subsection{Dateisystemisolierung durch den Mount"=\\Namespace}
			...
			% Filesystem Isolation kann mit user namespaces verstaekrt werden, indem user und groups IDs auf wenige rprivileigierte host uids und group ids gemappt werden. Zusammen mit dem mount namespace und einer pivot_root umgebung kann das den schutz fuer filesystembasierte Attacken erhoehen
  ...
  \section{Einschränkung von Zugriffsrechten}
    ...
    \subsection{Capabilities}
		\label{capabilities}
			...
			% TODO: "privilegierte Aktion" -- zuvor definieren, dass privilegiert == "Root-Rechte benötigt" ist.
		...
		\label{lsm}
      	...
      	% Optional, da auf MAC alzu sehr eingehen nicht zu sehr im Scope sein sollte.
				...
				%\paragraph{Analyse des Standardprofils}
					...
					\end{lstlisting}
					Diese Liste untersagt, abgesehen von einem Lesezugriff, alle anderen Privilegien aus dem bereits geschilderten Rechteset für das Verzeichnis \texttt{/sys/}. Die Ausnahme bilden effektiv Dateien und Unterverzeichnisse in \path{/sys/fs/cgroups/}. Damit ist es Containern möglich, Informationen über Control Groups des Hostsystems zu beziehen.
    ...
    \subsection{\emph{Seccomp}}
		\label{seccomp}
			...
			%Das hat die positive Auswirkung, dass \emph{Seccomp}-Profile auch von nicht-privilegierten Nutzern geladen werden können. Mit LSMs ist dies nicht möglich.
