...
\label{secLinux}
  ...
  \section{Einschränkungen von Zugriffsrechten}
    ...
    \subsection{Linux Security Modules (\acrshort{LSM}s) und Mandatory Access Control (\acrshort{MAC1})}
			...
			Aus diesem Grund überschreiben LSMs nicht DAC-Mechanismen, sondern ergänzen letztere, indem LSM-Checks erst durchgeführt werden, wenn die DAC den Zugriff gestattet hat. Durch diese Reihenfolge ist sichergestellt, dass die Nutzung einer LSM-Implementierung optional ist und unabhängig von der DAC funktioniert. Deswegen können auch Anwendungen, die das LSM-Framework nicht unterstützen, weiterhin funktionieren.
      	...
      	% Macht Sinn das erst am Ende zu machen, wenn noch Zeit ist. Weil SELinux im Detail mehr Exkurs wird.
				SELinux steht für \emph{Security Enhanced Linux} und wurde ursprünglich von der NSA entwickelt.

				SELinux ist die Implementierung einer feingranularen und flexiblen MAC (in Form eines Labeling-Systems).
				...
				Unterstützt Type Enforcement (TE), Role-Based Access Control (RBAC) und optional Multi-Level Security (MLS) / Multi-Category Security (MCS).

				Kann verwendet werden, um Prozesse auf "least privilege" einzuschränken. Schützt Integrität und Vertraulichkeit von Prozessen und Daten.

				SELinux bietet strenge Ressourcenisolierung und -beschränkung für Prozesse. \cite{redhatSec}

				Mit SELinux wird jedem Objekt im System ein Sicherheits-Label zugewiesen. Ein Objekt ist hierbei ein Sammelbegriff für alle Ressourcen, auf die sich der Zugriff beschränken lässt, also z.B. Prozesse, Dateien, Verzeichnisse und Netzwerkports. SELinux wertet diese Labels anhand einem definierten Regelwerk aus und entscheidet, ob die Operation fortgesetzt werden darf \cite{linuxSecOverview}. Die Regeln werden in der Summe auch als Policy bezeichnet.
				...
				\paragraph{Type Enforcement}

      \subsubsection{AppArmor}
      	% Optional, da auf MAC alzu sehr eingehen nicht zu sehr im Scope sein sollte.
				AppArmor ist ein MAC-Schema, das gebaut wurde, um eine einfache Handhabung zu realisieren. Es ist in den Linuxdistributionen \emph{Ubuntu} und \emph{OpenSUSE} standardmäßig zum Einsatz.

				Der fundamentale Unterschied zu SELinux ist, dass nicht Objekten keine Label zugeweisen werden, sondern allein Pfadnamen dazu dienen eine Sicherheits-Policy anzuwenden \cite{linuxSecOverview}.

				AppArmor unterstützt einen Lernmodus, indem das Verhalten einer Anwendungen beobachtet wird und daraus automatisch ein Sicherheitsprofil erstellt wird \cite{linuxSecOverview}.
