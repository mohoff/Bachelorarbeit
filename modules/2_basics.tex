...
\label{basics}
  ...
  \label{introVirt}
    ...
    \label{introVirtContainer}
      Container-basierte Virtualisierung wird vorrangig als leichtgewichtige Alternative zu der Hypervisor-basierten Virtualisierung gesehen\cite[S.2]{containerVirtPerformance}. Erstere nutzt direkt den Hostkernel, um mehrere parallele, virtuelle Umgebungen zu schaffen \cite[S.6]{dockerBook}. Ein Hypervisor wird in diesem Ansatz nicht benötigt \cite[S.7]{dockerBook}. Vielmehr wird das native System und dessen Ressourcen partitioniert, sodass mehrere virtuelle, voneinander isolierte Instanzen betrieben werden können, die als Container bezeichnet werden \cite[S.2]{containerVirtPerformance}\cite[S.1]{dockerSec1}. Die Isolation basiert auf dem Konzept von Namensräumen, die unter Linux \emph{namespaces} genannt werden. Diese, sowie \emph{cgroups}, die für das Ressourcenmanagement verantwortlich sind, werden in den Kapiteln \ref{secIsolierung} und \ref{secResLimit} genauer betrachtet.
			% TODO: eine Quelle fuer namespaces und control groups und deren aufgaben.
