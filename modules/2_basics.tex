...
\label{basics}
  ...
  \label{introVirt}
    ...
    Der Einsatz von Virtualisierung bietet vielfältige Vorteile für \acrshort{IT}-Unternehmen. In der folgenden Auflistung sind einige Problemfaktoren und Anforderungen an IT-Lösungen zusammengefasst, die alle durch Virtualisierungslösungen addressiert werden können \cite[S.1]{bsiVirt}\cite[S.662]{tanenbaumOS}.
		...
		% Probleme von Firmen:
		% Internetriesen muessen skalieren. Dort gibt es tausende von Servern, wo sich Einsparungsvorteile,Kosteneinsparung und Skalierungseffekte sehr auszahlen.
		% In Firmen generell: IT waechst, IT Portfolio waechst, auch historisch --> alte und neue Versionen, unterschiedliche Bibliotheken, legacy code, eine anwendung fuer windows server 2008, andere auf debian.
		% Anbieterwechsel in Firmen, die IT in Datencentern outsourcen.
		% Mehrere Kunden, die Serverressourcen brauchen, aber sich nicht beeinflussen duerfen.


		% TODO: Besserer Ansatz:
		% Probleme der IT erklaeren und wie Virtualisierung sie bekaemptf:
		% ----------
		% Vorteile: Isolation, weniger physische Maschinen noetig (spart geld (hw und energie, klimatisierung) und platz),
		% Setzen von Kontrollpunkten (Snapshots, Recovery)
		% Migrationsprozesse stark vereinfacht, z.b. wenn Anwendungen in neue Architektur gepresst werden mit LoadBalancing etc.
		% betriebssystemunabhengigkeit --> virtual appliance \cite[S.672f.]{tanenbaumOS}
		%			--> Anwendungen hängen in der Regel von zahlreichen Bibliotheken und anderen Anwendungen ab. Wenn Softwarelösungen verkauft werden, kommt es oft zu Inkompatiblitäten, da verschiedene Kernelversionen, Compilerversionen, Bibliotheksversionen interferieren. Virtual Appliances stellen eine Lösung dar, die die Anwedung in eine VM "eingeschweißt" wird und nicht mehr die einzelne Anwendung an den Kunden verkauft wird, sondern ein Gesamtpakekt in Form einer direkt ausführbaren Anwendungsumgebung, in der alle benötigten Komponenten kompatibel zueinander vorinstalliert sind.



		% ^		einige gruende aus \cite[S.662]{tanenbaumOS}
